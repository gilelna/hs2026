...
function hs_inline_video_shortcode($atts) {
    ...
    if (strpos($video_url, 'youtube.com') !== false || strpos($video_url, 'youtu.be') !== false) {
        ...
        foreach ($patterns as $p) {
            if (preg_match($p, $video_url, $m)) { $video_id = $m[1]; break; }
        }
    ...
    elseif (strpos($video_url, 'vimeo.com') !== false) {
        ...
        if (!empty($parts['query'])) {
            ...
            if (!empty($qs['h'])) { $vimeo_hash = $qs['h']; }
        }
        if (!$vimeo_hash && preg_match('/vimeo\.com\/\d+\/([A-Za-z0-9]+)/', $video_url, $m2)) {
            $vimeo_hash = $m2[1];
        }
        $type = $video_id ? 'vimeo' : '';
    }
    ...
    // JS consolidated into js/main.js; no per-shortcode enqueue here
